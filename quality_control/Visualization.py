...
def plot_FN_brain_surface_5view(brain_map: np.ndarray,
    ...
    fig, axs = plt.subplots(nrows=8, ncols=1, figsize=figure_size)
...
def plot_FN_brain_volume_3view(brain_map: np.ndarray,
    ...
    image_rgb = plot_voxel_map_3view(Overlay_Image_2, Map_2*0, center=Center, rotation=rotation, organization=organization, color_function=color_function)

    # settings for subplot
    fig, axs = plt.subplots(nrows=2, ncols=1, figsize=figure_size)
    ...
    axs[1].imshow(image_rgb)
    axs[1].axis('off')
    axs[1].set_title(label=figure_title, loc='center', pad=30, fontsize=60, fontweight='bold', fontname='Arial', color=(1, 1, 1))
